...
// const : 상수형 변수 , let : Scope가 명확한 변수 (지역변수)
/*
    스프레드 연산자 => 복사할 때 (... : 전체를 가져와라)
        const aaa=[1,2,3]
    ...
    bbb.map((data)=>{
        처음에 data='aaa', 다음 바퀴때는 'bbb', 그다음 'ccc'를 가져옴
    })
...
// ${} (EL) , {문자열}, {변수}
/*
    <App4 movie={movie} aaa={} bbb={}/> ==> 생성자의 매개변수에 값을 전송
    
    class App4 extends Component{
        constructor(props) {  => this.props.movie, this.props.aaa, this.props.bbb를 다 받을 수 있음
            ==> 값을 받을 때는 여기서 받아야함
            props, state => 멤버변수 (class영역 전체에서 사용할 수 있는 변수)
            props => 변경할 수 없는 변수
            state => 변경해서 사용하는 변수  (여러개의 데이터를 한번에 모아서 관리)
                var a=[]
                var b={}
                var c=1
